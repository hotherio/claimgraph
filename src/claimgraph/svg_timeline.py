...
def _layered_layout(node_ids: list[str], deps: list[tuple[str, str]]):
    ...
    def _depth(n, seen=frozenset()):
        if n in depth:
            return depth[n]
        ds = [d for d in deps_of.get(n, []) if d not in seen]
        depth[n] = 0 if not ds else 1 + max(_depth(d, seen | {n}) for d in ds)
        return depth[n]

    for n in node_ids:
        _depth(n)
    order = sorted(node_ids, key=lambda n: (depth[n], n))  # foundations first, then their dependents
...
function render(i){var st=statusAt[i];
 ...
 var sp=steps[i];readout.innerHTML='<span class="cg-year">'+sp.y+'</span><span class="cg-badge '+sp.badge+'">'+sp.blab+'</span><span class="cg-subj">'+sp.s.replace(/</g,"&lt;")+'</span>';
