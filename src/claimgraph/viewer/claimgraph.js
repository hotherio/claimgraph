...
(() => {
  ...
  async function load(src) {
    const seq = ++loadSeq;
    // src is either a URL (fetch) or, for a standalone export, the data object itself (no network).
    const data = (src && typeof src === "object") ? src : await fetch(src).then((r) => r.json());
  ...
  // A standalone export inlines its data in a <script type="application/json" id="cg-data"> island,
  // so the viewer renders with no network. The hosted site keeps the URL / example-switcher path.
  const inlineEl = document.getElementById("cg-data");
  const INLINE = inlineEl ? JSON.parse(inlineEl.textContent) : null;
  ...
  const initial = INLINE || (sel && sel.value) || window.CG_DATA || "assets/claimgraph.json";
  if (typeof initial === "string") setLede(initial);
  load(initial);
