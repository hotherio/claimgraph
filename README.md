...
- **Do the blueprint, the history, and the kernel agree?** Reconciling a Lean Blueprint, the CKC
  commit history, and a live `#print axioms` reading, `claimgraph` separates a claim's *validity*
  (what the kernel decides) from its blueprint *coverage* (whether everything it `\uses` is itself
  formalized). See [Reconcile a blueprint](#reconcile-a-lean-blueprint-with-the-kernel) below.
...
## Reconcile a Lean Blueprint with the kernel

The commit history is one record of a project; a [Lean Blueprint](https://github.com/PatrickMassot/leanblueprint)
is another. `claimgraph` reads a blueprint and lines up **three readings** of every claim:

| Reading | Source | Means |
| --- | --- | --- |
| `claimed` | blueprint `\leanok` | what the author asserts is formalized |
| `asserted` | CKC commit `Status:` | what the recorded history says |
| `kernel` | `#print axioms` (live, via `axiom-report`) | what the proof assistant actually checks |

```bash
# Import a blueprint as a graph and ground its \leanok against the kernel
claimgraph blueprint blueprint/src/content.tex --project lean/

# Reconcile blueprint + commit history + kernel; report where they disagree
claimgraph reconcile blueprint/src/content.tex . --project lean/

# Honesty gate (for CI): exit nonzero on a validity gap; --strict also fails on coverage gaps
claimgraph audit blueprint/src/content.tex --project lean/
```

The kernel reading is obtained from [`axiom-report`](https://github.com/hotherio) (set
`--axiom-report PATH` or `$CKC_AXIOM_REPORT`); pass `--axioms FILE` to ground against a saved report
instead of a live build, or omit `--project` to leave the kernel column ungrounded.

Each node is then classified by where the readings agree:

- **`kernel-refutes-claim`** — a *validity* gap: a claim marked proved (`\leanok` or
  `Status: math.machine-checked`) that `#print axioms` shows resting on `sorryAx` or a cited axiom.
  This is what `audit` fails on by default.
- **`blueprint-incomplete`** — a *coverage* gap: a claim that is itself machine-checked but `\uses`
  a concept never modelled in Lean (a prose-only definition). Not a hole; a faithfulness note.
  `audit --strict` also fails on these.
- **`consistent`**, plus `undocumented`, `stale-blueprint`, `paper-only`, `ungrounded` for the
  remaining combinations.

Validity and coverage are kept on separate axes on purpose: a blueprint `\uses` edge is expository,
not a logical dependency, so it never drags a kernel-clean theorem's validity down. Two bundled
examples exercise this: [`examples/blueprint-gap/`](examples/blueprint-gap/) (a synthetic blueprint
paired with a saved axiom report, isolating each category) and
[`examples/blueprint-igl/`](examples/blueprint-igl/) (a real blueprint reconciled against the
[paper-igl](https://github.com/hotherio/paper-igl) history).

```bash
claimgraph reconcile examples/blueprint-igl/content.tex --from-fixture examples/paper-igl/paper-igl.commits \
  --axioms examples/blueprint-igl/axioms.txt
```

The reconciliation viewer is [`docs/blueprint.html`](docs/blueprint.html); node colour is effective
status, and a red double ring marks a validity gap.
...
target, relation, breaking). `reconcile` / `blueprint` add the per-node reconciliation fields
(`lean`, `aliases`, `claimed`, `asserted`, `kernel`, `agreement`, `blueprint_complete`, `uses_gap`),
emitted only when set so a commit-only graph keeps the same shape.
